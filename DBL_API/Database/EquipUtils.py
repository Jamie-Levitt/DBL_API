...
from Types import Equipment, EquipRarity, TraitRarity
...
def __getEquipTags(APIConn:DBAPIConnection, equipID:int) -> Equipment:
    soup = APIConn.loadPageData('EQUIP', equipID)
...
def downloadEquipData(APIConn:DBAPIConnection) -> list[Equipment]:
    soup = APIConn.loadPageData('EQUIP_ROOT')
    ...
    for i in range(len(baseRawEquipList) - 1):
        ...
        img_path = APIConn.relAssetPath(f'equips{os.sep}{id}.png')
        eData = __getEquipTags(APIConn, id)
        conditions = eData['conditions']
        effect1 = eData['effects'][0]
        effect2 = eData['effects'][1]
        effect3 = eData['effects'][2]
